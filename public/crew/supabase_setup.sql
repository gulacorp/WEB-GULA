...
-- 5. Contactos / leads (formulario de contacto general)
create table if not exists public.contactos (
  id          uuid primary key default gen_random_uuid(),
  nombre      text,
  email       text not null,
  telefono    text,
  tipo        text default 'contacto',       -- 'contacto', 'catering', 'prensa'
  mensaje     text,
  ciudad      text,
...
-- 6. Franquicias - tabla exclusiva para solicitudes de franquicia
create table if not exists public.franquicias (
  ...
  email             text not null,
  telefono          text,
  ciudad            text not null,           -- ciudad donde quiere abrir
  inversion         text,                    -- rango de inversión
  experiencia       text,                    -- si tiene experiencia
...
drop policy if exists "Insertar franquicias" on public.franquicias;
create policy "Insertar franquicias" on public.franquicias
  for insert with check (true);
